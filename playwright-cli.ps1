...
# accepted rather than replaced; a bootstrap installs Node 22 LTS.
$MinNodeMajor = 18
...
function Redact-Url([string]$Url) { return ($Url -replace '//[^/@\s]*@', '//***@') }
...
function Redact-Log([string]$Path) {
    ...
    try {
        ...
        Set-Content -LiteralPath $Path -Value ($raw -replace '//[^/@\s]*@', '//***@') -NoNewline
    } catch {
        # A log we cannot rewrite must not be printed at all.
...
function Try-Node([string]$Candidate) {
    ...
    $script:NodeBinDir = Split-Path -Parent $Candidate
    return $true
}
...
function Install-Node {
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath (Join-Path $extracted "node.exe") -PathType Leaf)) {
            Die $ExNodeBootstrap "the Node archive is missing node.exe"
        }
        ...
        # The validated tree moves into place in one step: a half-extracted
        # directory left by an interrupted run would otherwise be picked up as
        # "usable" by the next invocation.
    ...
    if (-not (Try-Node (Join-Path $Prefix "node\node.exe"))) {
        Die $ExNodeBootstrap "the Node just installed at $Prefix\node does not run on this host"
    }
...
$logPath = Join-Path $Prefix "install.log"
...
if (-not (Resolve-Npm)) {
    Die $ExNodeBootstrap ("found Node at $script:NodeExe but no npm beside it or on PATH; " +
        "install npm (it ships with Node) and re-run")
}
...
function Invoke-NpmInstall {
    ...
    try {
        ...
        $ErrorActionPreference = 'Continue'
        & cmd.exe /c "`"$script:NpmCmd`" install --global `"$spec`"" > $logPath 2>&1
...
function Get-FailureClass {
    ...
    if ($log -match '(?i)cdn\.playwright\.dev|playwright\.azureedge\.net|Failed to (download|install) (browser|chromium|firefox|webkit)') {
...
try {
    ...
    $ErrorActionPreference = 'Continue'
    $versionOut = (& cmd.exe /c "`"$wrapper`" --version" 2>$null | Select-Object -First 1)
    if ([string]::IsNullOrWhiteSpace($versionOut)) {
        & cmd.exe /c "`"$wrapper`" --help" > $null 2>&1
