...
- **Positionals are checked too, not just flags.** A page verb originally fell
  straight through to "approved" once its verb matched, with its arguments
  unread. That made `goto file:///<path>` auto-approved, which is not a page
  action: the file lands in the page and the next `snapshot` prints it into the
  agent's context, i.e. an arbitrary local file read behind the one gate whose
  job is to refuse those. A URI-shaped argument now passes only as plain
  http(s), and only to a host that is not link-local -- tested by netmask via
  `ipaddress`, so the whole cloud-metadata range and its alternate spellings are
  covered instead of one hard-coded address. Loopback stays allowed on purpose,
  because previewing a local dev server is the workflow this feature exists for.
