...
export function useComposerDraft(opts: ComposerDraftOptions = {}): ComposerDraft {
   ...
   * effect that makes the second pass see state the first pass already changed — it would
   * find the memo gone, fall back to inferring the base from the text, and drop the
   * punctuation the memo exists to preserve.
   */
