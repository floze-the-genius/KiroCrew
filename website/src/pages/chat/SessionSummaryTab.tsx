...
export default function SessionSummaryTab({ slot }: { slot: string }) {
  ...
  if (error) {
    ...
    // not-generated states use, and a Retry. Without it a failure rendered as a
    // single grey sentence — quieter than either harmless empty state, so the
    // one state with something to recover looked the least serious. The header's
    // reload button is not a substitute: this branch returns before the header
    // renders, so a failure took away the only control that could fix it.
    return (
      <Centered>
        ...
        <button
          type="button"
          onClick={() => refetch()}
          className="mt-1 inline-flex items-center gap-1.5 text-[12px] px-2.5 py-1 rounded-md border border-border-strong bg-card text-text hover:bg-bg-hover"
        >
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      ...
      {(data?.constraints?.length ?? 0) > 0 && (
        <div className="shrink-0 border-t border-border-strong bg-panel">
          <button
            type="button"
            onClick={toggleNotes}
      ...
      <div className="shrink-0 border-t border-border-strong px-3 py-2 bg-panel-strong flex items-center">
        <span className="text-[11px] text-muted-strong font-mono">
          {/* ONE freshness verdict, in one place. Staleness used to sit in the
              header opposite this line, which made the reader reconcile two
              signals at opposite corners — and the longer marker wrapped, pushing
              the count chip out of position. Rendered as a single interpolated
 ...
 *  still; that needs the #2855 payload and is tracked as a follow-up. */
