...
export function useInstanceFormState(initial: InstanceFormValues, usedPorts: number[]) {
  ...
  const body = useCallback(
    ({
      ...
      omitIdentity = false,
    }: { explicitClears?: boolean; omitIdentity?: boolean } = {}): AddInstanceBody => {
      const v = values
      ...
      return {
        ...
        ...(ssm
          ? {
              ...(omitIdentity ? {} : { ssm_target: v.ssmTarget.trim() }),
...
export function EditInstanceForm({
  ...
  onCancel: () => void
  /**
   * Freeze the fields that identify the machine (connection method, SSM target).
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      api.updateInstance(
        inst.id,
        form.body({ explicitClears: true, omitIdentity: lockTransport }),
      ),
    onSuccess: onSaved,
  })
      ...
      {isSsm ? (
        <>
          <label htmlFor={`${idPrefix}-ssm-target`} className="flex flex-col gap-1 text-[13px] text-muted">
            ...
            <input id={`${idPrefix}-ssm-target`} aria-label={i18nT('pages.settings.instancesPanel.ssm_target_instance_id')} className={inputCls} value={values.ssmTarget} onChange={e => set('ssmTarget', e.target.value)} placeholder="i-0123456789abcdef0" readOnly={lockTransport} />
            <span className="text-[12px] text-muted leading-snug">
