...
describe('kiroGhost style', () => {
  it('uses the shipped mark paths verbatim', () => {
    const asset = readFileSync(MARK, 'utf8')
    // Exact substring match: a reformat or a nudged control point in the asset
    // fails this rather than silently leaving the generator on the old shape.
    expect(asset).toContain(BODY)
    expect(asset).toContain(EYE_A)
    expect(asset).toContain(EYE_B)
  })
