  ...
  const openSession = useCallback(
    async ({ repoRef, number, kind = 'issue', verb, title, prompt, existing }: OpenSessionArgs): Promise<InvestigationRecord | null> => {
      ...
      try {
        ...
        try {
          const res = await issueRadarApi.saveInvestigation(
            repoRef, number, { status: 'investigating' }, kind, verb, slot.key,
          )
          running = res.investigation
        } catch {
          // Leave the record as the claim left it; the link is what makes the session
          // reachable, and it is already stored.
        }
