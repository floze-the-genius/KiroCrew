  ...
  return (
    <div className="relative flex flex-col h-full min-h-0 bg-bg">
      ...
      >
        ...
        {/* Overflow, not a fifth control in this row.
            `max-two-buttons-per-row` gives a row that already carries 3+ on the
            base branch legacy status but forbids it GROWING, and prescribes collapsing
            rather than widening. This row already had back / forward / expand /
            preview-size, so the browser-view toggle goes behind the standard
            overflow trigger -- the pattern copied from `CronRowActions.tsx`,
            which the rule names. A menu trigger counts as one control. */}
        <DropdownMenu open={moreOpen} onOpenChange={setMoreOpen}>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className={`${iconBtn} ${viewRunning ? 'text-accent' : ''}`}
