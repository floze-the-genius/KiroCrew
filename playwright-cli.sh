...
# changing the mode.
: >"$LOG" 2>/dev/null || die "$EX_NOT_WRITABLE" "cannot write the install log $LOG"
...
_resolve_npm || die "$EX_NODE_BOOTSTRAP" \
  "found Node at $NODE but no npm beside it or on PATH; install npm (it ships with Node) and re-run"
...
_sanitize_log() {
  [ -f "$LOG" ] || return 0
  if _redact_urls <"$LOG" >"$LOG.redacted" 2>/dev/null \
     && mv -- "$LOG.redacted" "$LOG"; then
    chmod 600 -- "$LOG" 2>/dev/null || true
    return 0
  fi
  # Redaction could not run (a full filesystem, an unreadable log). The log may
  # carry a token, so it is EMPTIED rather than left for _show_log_tail to print:
  # failing open here would defeat the redaction entirely.
  rm -f -- "$LOG.redacted"
  : >"$LOG" 2>/dev/null || true
  LOG_SUPPRESSED=1
}
