...
def headless_auth_warning(environ: "Mapping[str, str] | None" = None) -> str:
    ...
    remedy = (
        f"touch {dotenv} && chmod 600 {dotenv}\n"
        f"     printf '%s\\n' \"{_AUTH_ENV_VAR}=${_AUTH_ENV_VAR}\" >> {dotenv}"
    )
    lines = [
...
def _names_defined_in_env_file(path: Path) -> "set[str]":
    """Return the variable NAMES assigned in a ``.env`` file.

    Mirrors ``load_credentials()``'s parse (strip, skip blanks and ``#``
    comments, split on the first ``=``) so this agrees with what the gateway
    will actually resolve. Values are discarded without being inspected. An
    unreadable or absent file yields an empty set, which makes the caller warn —
    the safe direction, since a missed warning is the defect being fixed.
    """
    try:
