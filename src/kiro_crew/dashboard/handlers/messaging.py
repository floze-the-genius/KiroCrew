...
def _deny_browser_app_token(request: web.Request, operation: str) -> web.Response | None:
    """Refuse app tokens on the browser MUTATION endpoints. 403, or None to proceed.

    The middleware's app-scope check only proves the ROUTE is in the calling app's
    manifest ``permissions.api`` allowlist. It says nothing about whether an app
    should be able to arm this capability, and these three handlers are exactly
    the wrong things for an app to reach:

    * installing the CLI globally is what ACTIVATES browser auto-approval, so an
      app could hand the agent an unprompted browser on a host that had none;
    * the attach token is a stored credential that silences the browser's own
      per-attach approval prompt, i.e. the last human checkpoint before a program
      drives the user's logged-in session;
    * a browser download writes to the machine.

    Reads (``GET``) stay open to a scoped app: knowing whether browsing exists is
    not a capability. Mirrors the ComputerUse keystone precedent, which denies app
    tokens on its write path for the same reason.
    """
    ...
    _sel().log_api_access(
        ...
        error="app tokens may not install the browser CLI or set its attach token",
    )
    return web.json_response(
...
async def api_browser_token_put(request: web.Request) -> web.Response:
    ...
    next gateway start.
    """
    denied = _deny_browser_app_token(request, "browser_token_set")
...
async def api_browser_install_start(request: web.Request) -> web.Response:
    ...
    """
    denied = _deny_browser_app_token(request, "browser_cli_install")
    if denied is not None:
        return denied
    state: DashboardState = request.app["state"]
    task = getattr(state, "_browser_install_task", None)
...
async def api_browser_engine_install(request: web.Request) -> web.Response:
    ...
    """
    denied = _deny_browser_app_token(request, "browser_engine_install")
    if denied is not None:
        return denied
    state: DashboardState = request.app["state"]
    try:
...
async def api_browser_view_get(request: web.Request) -> web.Response:
    ...
    """
    denied = _deny_browser_app_token(request, "browser_view_status")
    if denied is not None:
        return denied
...
async def api_browser_view_start(request: web.Request) -> web.Response:
    ...
    the GET carries.
    """
    denied = _deny_browser_app_token(request, "browser_view_start")
    if denied is not None:
        return denied
