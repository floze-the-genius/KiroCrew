...
def _load_shareability_state() -> tuple[
    ...
    Preflight entries are keyed by execution identity, but the row builder knows
    only a name, so the newest measurement per name is used. A mismatch is
    self-correcting — the next explicit probe re-derives under the right key.
    """
    try:
