...
def _is_safe_browser_cli_argument(arg: str) -> bool:
    ...
    Flags were validated from the start; positionals were NOT, and a page verb
    reached a bare ``continue``. That made navigation to a non-http scheme
    auto-approved, which is not "a page action" at all:
    ...
    * ``view-source:`` does both.
    ...
    A URI-shaped argument passes only as plain http(s) to a non-link-local host;
    anything else falls through to interactive approval. Loopback is deliberately
    ALLOWED -- previewing a local dev server is a primary workflow -- but
    link-local is refused, because that range is where cloud instance metadata
    (and its credentials) lives. Tested by netmask via ``ipaddress`` rather than
    matched against one hard-coded address, so the whole 169.254/16 range and
    IPv6 fe80::/10 are covered, alternate encodings included.
    """
...
def _is_browser_cli_command(tool_title: str) -> bool:
    ...
    for seg in segments:
        ...
        if verb in _BROWSER_CLI_PAGE_VERBS:
            # Positionals are checked too. This used to be a bare `continue`,
            # which auto-approved `goto file:///...` -- see
