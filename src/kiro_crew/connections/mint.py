...
async def _dispose_mint(entry: MintState) -> None:
    ...
    try:
        ...
        _remove_mint_agent_spec(spec_path)
...
async def start_oauth_mint(
    ...
    if await asyncio.to_thread(grant_present, mcp_url):
        ...
        _log_mint_outcome(slug, "ok", "reason=already_granted")
        return
    ...
    try:
        ...
        client = acp_client_cls(
            work_dir=data_home() / "connections" / "mint",
            model="auto",
            agent=agent_name,
            sandbox_mode="auto",
        ...
        if oauth_url and oauth_url_contains_credential(oauth_url):
            ...
            _log_mint_outcome(slug, "error", "reason=mint_url_rejected")
            return
        ...
        entry_missing = _agent_spec_entry_missing(slug) if not oauth_url else False
        superseded = False
        ...
        if holdings:
            ...
            holdings = {}
        if superseded:
            _log_mint_outcome(slug, "ok", "reason=superseded")
            return
        if entry_missing:
            _log_mint_outcome(slug, "error", "reason=mint_server_absent")
        else:
            _log_mint_outcome(slug, "ok", f"url_minted={bool(oauth_url)}")
        ...
        _log_mint_outcome(slug, "error", f"reason={reason}")
    finally:
