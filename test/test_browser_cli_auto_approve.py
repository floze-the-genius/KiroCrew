...
def test_plain_http_navigation_including_loopback_stays_approved():
    """Loopback is deliberately allowed: previewing a local dev server is a
    primary workflow, and denying it would defeat the feature for the case it
    exists to serve."""
    for command in (
        "playwright-cli goto https://example.com",
        "playwright-cli goto http://127.0.0.1:5173",
        "playwright-cli goto http://localhost:8080/path?q=1",
...
def test_a_link_local_address_is_denied_by_netmask_not_by_one_literal():
    ...
    range. Checked with `ipaddress`, so the whole range and its alternate
    spellings are covered rather than one hard-coded address."""
