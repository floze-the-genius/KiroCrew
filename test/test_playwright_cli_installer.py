...
def _run(
    ...
    *args: str,
    ...
    return run_bounded(["sh", str(INSTALLER_SH), *args], env=env)
...
def test_powershell_installer_parses(tmp_path: Path) -> None:
    ...
    ps_home = tmp_path / "pshome"
    ps_home.mkdir()
    env = dict(os.environ)
    env.update(
        {
            "HOME": str(ps_home),
            "USERPROFILE": str(ps_home),
            "XDG_CACHE_HOME": str(ps_home / "cache"),
            "XDG_CONFIG_HOME": str(ps_home / "config"),
            "XDG_DATA_HOME": str(ps_home / "data"),
...
def test_wrapper_generation_escapes_the_paths_it_embeds(
    ...
    ran = subprocess.run([str(wrapper), "--version"], capture_output=True, text=True, check=False)
...
def test_the_windows_script_scrubs_its_log_too() -> None:
    ...
    assert failure_block.index("Redact-Log $logPath") < failure_block.index(
        "Get-FailureClass"
    ), "the log must be scrubbed before it is classified and tailed"
