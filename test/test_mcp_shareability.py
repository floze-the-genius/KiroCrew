...
def _key(**over: object) -> vc.CacheKey:
    base: dict = {
        "server_name": "srv",
...
class TestNoBlockingRecordIoOnTheLoop:
    ...
    def test_the_blocking_set_is_actually_derived(self) -> None:
        ...
        assert {"load_cache", "load_ledger", "cache_key_for"} <= found, found
    ...
    def test_cache_exposes_names_without_leaking_its_key_encoding(self) -> None:
        cache = vc.VerdictCache(vc.cache_path(Path("/nonexistent")))
        cache.put(_key(server_name="a"), _verdict())
        cache.put(_key(server_name="b", command_args_hash="other"), _verdict())
...
class TestBackendRecordsHazards:
    ...
    @staticmethod
    def _backend(server_name: str, *, exclusive_token: str = "") -> object:
        """A Backend-shaped stand-in for the two fields ``_record_hazard`` reads.

        Deliberately not a MagicMock: the point is to pin that the method reads
        ``pool_key.server_name`` and ``exclusive_token`` — the REAL field names
        on ``Backend`` — so a rename cannot leave this passing against a mock
        that would answer to anything.
        """
