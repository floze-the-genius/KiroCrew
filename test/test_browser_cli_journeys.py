...
def test_every_browser_route_has_a_deliberate_app_token_stance():
    ...
    # True = must refuse an app token. False = deliberately open, with the reason.
    EXPECTED = {
    ...
    for name, must_deny in EXPECTED.items():
        guarded = "_deny_browser_app_token" in inspect.getsource(getattr(messaging, name))
        if guarded is not must_deny:
            wrong.append(f"{name}: guarded={guarded} expected={must_deny}")
    assert not wrong, "app-token stance does not match the declared intent: " + "; ".join(wrong)
